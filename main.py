    ...
    async def on_message(update: Update, context):
        ...
        if not bootstrap.is_bootstrapped():
            ...
            if stage == "not_started":
                stage = "background"
            # 简化：将用户消息作为输入传递
            result = await bootstrap.process_stage(stage, {"raw_input": user_text})
...
async def async_main(args):
    ...
    bot_app = None
    if app["telegram"]:
        bot_app = await run_telegram_loop(app)

    scheduler_task = asyncio.create_task(run_scheduler(app, stop_event))
